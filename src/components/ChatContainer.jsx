  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500 '>
            ...
            <img src={assets.arrow_icon} alt="arrow" className='md:hidden max-w-7' />
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
            {messages.map((message, index) => (
                <div key={index} className={`flex ${message.senderId !== authUser._id && 'flex-row-reverse'} flex justify-end items-end gap-2`}>
                    {message.image ? (
                        <img src={message.image} alt='img' className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
                    ) : (
                    ...
                    <div className='text-center text-xs'>
                        <img src={message.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="user" className='w-7 rounded-full' />
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                <input onChange={(e) => setInput(e.target.value)} value={input} onKeyDown={(e) => e.key === 'Enter' ? handleSendMessage(e) : null} type="text" placeholder='Send a message...' className='flex-1 p-3 text-sm border-none rounded-lg outline-none text-white placeholder-gray-400' />
